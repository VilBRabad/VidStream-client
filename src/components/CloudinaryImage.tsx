...
const CloudinaryImage: React.FC<CloudinaryImageProps> = ({ movie }) => {
    ...
    const movFromStore = useSelector((store: IStore)=>store.watchlist.movies);
    ...
    return (
        <div className="span-start overflow-hidden relative moviecard h-full poster cursor-pointer">
            <AdvancedImage onClick={()=>navigate(`/movie/${movie._id}`)} className="poster-image absolute inset-0 w-full h-full object-cover transition duration-200 ease-in-out" cldImg={img} />
